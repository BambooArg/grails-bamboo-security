...
class AutoLoginServiceSpec extends Specification {
    ...
    void "test autoLogin method"() {
        ...
        def autoLoginEncrypter = Mock(AutoLoginEncrypter)
        autoLoginEncrypter.desEncrypt(_) >> { String hash ->
            new AutoLoginAuthenticationToken("hash", "Principal", null)
        }

        def authenticationDetailsSource = Mock(AuthenticationDetailsSource)
        authenticationDetailsSource.buildDetails(_) >>{ HttpServletRequest request1 ->
            new WebAuthenticationDetails(request1)
        }

        when: "call the autologin method"
        autoLoginService.autoLoginEncrypter = autoLoginEncrypter
        autoLoginService.authenticationDetailsSource = authenticationDetailsSource
